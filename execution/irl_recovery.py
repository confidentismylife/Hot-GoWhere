"""Inverse Reinforcement Learning — recover reward functions from behavior data.

STATUS (route-A, 2026-08): the MaxEnt IRL pipeline below is retained as an
experimental component, but the deployed reward weights are NOT taken from
it. Evidence: trajectories in data/trajectories/ were generated by a rule-
based Oracle (not LLM behavior), personas with <10 trajectories fall back
to the hardcoded table in _default_weights(), and the MaxEnt solution for
the remaining personas collapsed to safety-dominated vectors (reward
non-identifiability, cf. Skalse & Abate 2024; plus collinear features:
comfort is an alias of safety, social/conformity share one source field).
The deployed weights are hand-designed (see data/irl_weights.json
_provenance) and must be justified by sensitivity analysis.

Original cascade intent (kept for reference):
...
class IRLRecovery:
    ...
    def _default_weights(persona: str) -> np.ndarray:
        """Hand-designed fallback weights (route-A).

        These are NOT learned values; they mirror the manually designed
        literature-based weights in data/irl_weights.json. Used when a
        persona has insufficient trajectories (<10) for MaxEnt fitting.
        Feature order: safety, efficiency, social, conformity, comfort.
        """
        defaults = {
            "untrained_elderly": [0.45, 0.20, 0.10, 0.15, 0.10],
            "untrained_young": [0.35, 0.35, 0.10, 0.10, 0.10],
            "trained_staff":   [0.30, 0.30, 0.15, 0.20, 0.05],
            "guide":           [0.30, 0.25, 0.20, 0.20, 0.05],
            "firefighter":     [0.20, 0.45, 0.20, 0.05, 0.10],
        }
